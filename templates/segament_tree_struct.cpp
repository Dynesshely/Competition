  ...
  public:
    ...
    inline void Modify(i64 x, i64 l, i64 r, i64 v) {
        ...
        if (l > mid)
            ...
            Modify(s[x].ls, l, mid, v), Modify(s[x].rs, mid + 1, r, v);
        Update(x);
    }
    ...
    inline i64 Query_Sum(i64 x, i64 l, i64 r) {
        ...
        i64 mid = Average(s[x].l, s[x].r);
        if (l > mid)
            return Query_Sum(s[x].rs, l, r);
        else if (r <= mid)
            return Query_Sum(s[x].ls, l, r);
        else
            return Query_Sum(s[x].ls, l, mid) + Query_Sum(s[x].rs, mid + 1, r);
    }
    ...
    inline i64 Query_Mul(i64 x, i64 l, i64 r) {
        ...
        i64 mid = Average(s[x].l, s[x].r);
        if (l > mid)
            return Query_Mul(s[x].rs, l, r);
        else if (r <= mid)
            return Query_Mul(s[x].ls, l, r);
        else
            return Query_Mul(s[x].ls, l, mid) * Query_Mul(s[x].rs, mid + 1, r);
    }
} st;
