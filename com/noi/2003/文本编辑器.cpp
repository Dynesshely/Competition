/*
    ...
    操作名称    输入文件中的格式	功能
    Move    将光标移动到第 kk 个字符之后，如果 k=0k=0，将光标移到文本开头
    Insert  在光标处插入长度为 nn 的字符串 ss，光标位置不变n\geq1n≥1
    Delete  删除光标后的 nn 个字符，光标位置不变，n \geq 1n≥1
    Get     输出光标后的 nn 个字符，光标位置不变，n \geq 1n≥1
    Prev    光标前移一个字符
    Next    光标后移一个字符
    你的任务是：
    建立一个空的文本编辑器。
