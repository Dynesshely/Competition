...
//     for (int i = 0; i < 22; ++i)
//         for (int j = 0; j < 22; ++j)
//             arr[i][j] = MAXINT, via[i][j] = 1;
//     for (int i = 1, j = 1; i <= n && j <= m; i = j == m ? i + 1 : i, j = j == m ? 1 : j + 1) {
//         scanf("%d", &arr[i][j]);
//         via[i][j] = 0;
//         if (arr[i][j] < MIN)
//             MIN = arr[i][j], nx = j, ny = i;
//     }
//     while (via[ny - 1][nx] == 0 || via[ny + 1][nx] == 0 || via[ny][nx - 1] == 0 || via[ny][nx + 1] == 0) {
//         printf("(%d,%d) ", ny, nx);
//         via[ny][nx] = 1;
//         int u = arr[ny - 1][nx], d = arr[ny + 1][nx], l = arr[ny][nx - 1], r = arr[ny][nx + 1];
//         if (via[ny - 1][nx] == 0 && u < l && u < r && u < d)
//             ny--;
//         else if (via[ny + 1][nx] == 0 && d < l && d < u && d < r)
//             ny++;
//         else if (via[ny][nx - 1] == 0 && l < d && l < r && l < u)
//             nx--;
//         else if (via[ny][nx + 1] == 0 && r < l && r < d && r < u)
//             nx++;
//         else
//             break;
//     }
