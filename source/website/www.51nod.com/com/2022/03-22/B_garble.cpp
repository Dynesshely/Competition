...
using namespace std;
char g_gc() {
    static char b_buf[1 << 16], *s_s, *t_t;
    if (s_s == t_t) {
        t_t = (s_s = b_buf) + f_fread(b_buf, 1, 1 << 16, s_stdin);
        if (s_s == t_t)
            return (ReplacementFor_EOF);
    }
    return (*s_s++);
}
int r_read() {
...
void w_wt(int x_x) {
    if (x_x > 9)
        w_wt(x_x / 10);
    p_pc(((char)(0x88f + 7786 - 0x26c9)) + x_x % 10, 0);
}
void w_wts(int x_x, char o_op) {
    if (x_x < 0)
        p_pc(((char)(0x2000 + 516 - 0x21d7)), 0), x_x = -x_x;
    w_wt(x_x);
    p_pc(o_op, 0);
}
...
void a_add(int x_x, int y_y) {
    for (; x_x <= n_n; x_x += l_lb(x_x))
        s_sum[x_x] += y_y;
}
int a_ask(int x_x) {
    int a_ans = 0;
    for (; x_x; x_x -= l_lb(x_x))
        a_ans += s_sum[x_x];
    return (a_ans);
}
int main() {
    ...
    while (ReplacementFor_T--) {
        ...
        for (int i_i = 1; i_i <= n_n; i_i++) {
            ...
            if (m_mn == a_a[i_i])
                e_ept++, t_tt[a_a[i_i]]++, s_s.i_insert(a_a[i_i]);
            t_t[a_a[i_i]]++;
            if (t_t[a_a[i_i]] == 1)
                a_add(a_a[i_i], 1);
            v_val += a_ask(a_a[i_i] - 1);
            v_vector<int> v_v;
            for (auto i_it : s_s) {
                if (i_it > m_mn && i_it < a_a[i_i]) {
                    t_tt[i_it]--;
                    if (t_tt[i_it] == 0)
                        v_v.p_push_back(i_it);
                    e_ept--;
                }
                if (i_it > a_a[i_i])
                    break;
