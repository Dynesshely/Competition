...
bool ans[N * 2]; char sc[N];
int n, m, l, mxl, sl, k, laord[N], lac, cuord[N], cuc, ordi[N], ordj[N];
vector<int> b[N], lst[N]; vector<bool> a[N];
...
struct data{ int mx, mxp, c; };
inline data operator+(data a, data b){
    if(!a.c) return b; if(!b.c) return a;
    if(a.mx + b.c > b.mx)
...
struct segment_tree{
    ...
    inline data Query(int l, int r){
        ...
        for(l += D - 1, r += D + 1; l ^ r ^ 1; l >>= 1, r >>= 1){
            if(~l & 1) resl = resl + s[l ^ 1];
            if(r & 1) resr = s[r ^ 1] + resr;
        }
        return resl + resr;
    }
}t;
inline bool Calc(int m, int r){
    ...
    if(!tmp.c) return 1;
    if(tmp.mx > m) return 0;
    int i = ordi[tmp.mxp], j = ordj[tmp.mxp];
    if(~lst[i][j]) t.Insert(b[i][lst[i][j]], lst[i][j]);
    bool flag = Calc(j - 1, tmp.mxp - 1);
    if(tmp.mx == m){
        if(flag){
            for(int k = j; k <= tmp.mx; ++ k) ans[k] = 1;
            return 1;
        }
        if(~lst[i][j]) t.Delete(b[i][lst[i][j]]);
        return 0;
    }else if(tmp.mx < m){
        if(flag) for(int k = j; k <= tmp.mx; ++ k) ans[k] = 1;
        else{
            if(~lst[i][j]) t.Delete(b[i][lst[i][j]]);
            Calc(j, tmp.mxp - 1);
            for(int k = j + 1; k <= tmp.mx + 1; ++ k) ans[k] = 1;
        }
        return 1;
    }
}
int main(){
    scanf("%d", &n);
    for(int i = 1; i <= n; ++ i){
        scanf("%s", sc); l = strlen(sc);
        a[i].resize(l), b[i].resize(l), lst[i].resize(l);
        for(int j = 0; j < l; ++ j) a[i][j] = sc[l - j - 1] - '0';
        mxl = max(mxl, l), sl += l;
    }
    iota(laord + 1, laord + 1 + n, 1); lac = n; k = 0;
    for(int j = 0; j < mxl; ++ j){
        cuc = 0; for(int id = 1, i; id <= lac; ++ id){
            i = laord[id]; if(j < a[i].size() && !a[i][j])
    ...
    for(int i = 1; i <= n; ++ i){
        for(int j = 0, la = -1; j < a[i].size(); ++ j)
            if(a[i][j]) lst[i][j] = la, la = j;
        t.Insert(b[i].back(), a[i].size() - 1);
    }
    Calc(n + mxl, sl);
    for(int i = n + mxl, f = 0; ~i; -- i)
        if(f = ans[i] ? 1 : f) putchar(ans[i] + '0');
    putchar('\n');
    return 0;
}
