...
struct segment_tree {
    ...
    inline void Update(long long l, long long r, int a, long long L, long long R, int &k) {
        if (!k)
            k = ++t;
        if (l <= L && R <= r) {
            lz[k] = a;
        ...
        if (l <= M)
            Update(l, r, a, L, M, ls[k]);
        if (r > M)
            Update(l, r, a, M + 1, R, rs[k]);
    }
    inline int Query(long long p, long long L, long long R, int k) {
        if (!k)
            return n + 1;
        if (L == R)
            return lz[k] ? lz[K] : n + 1;
