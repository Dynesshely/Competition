...
function closeDropdown(e) {
  if (!e.target.closest(".search-area")) showDropdown.value = false;
}
...
function setCookie(n, v, d) {
  ...
  document.cookie =
    n + "=" + v + ";path=/;expires=" + dt.toUTCString() + ";SameSite=Lax";
}
...
onMounted(() => {
  theme.value = getCookie("academia-theme") || "system";
...
function cycleTheme() {
  ...
  setCookie("academia-theme", theme.value, 365);
  applyTheme(theme.value);
}
...
<template>
  <nav
    class="sticky top-0 z-100 flex items-center justify-between h-13 px-4 sm:px-6 border-b backdrop-blur-md transition-theme"
    :style="{
  ...
  >
    <router-link
      to="/"
      class="font-bold text-lg no-underline whitespace-nowrap tracking-tight"
      style="font-family: var(--font-heading); color: var(--color-fg)"
    >
      <BookOpen
        :size="18"
        class="inline-block mr-3 -mt-0.5"
        style="color: var(--color-accent)"
      />Dynesshely's OI Solutions
    </router-link>

    <div class="flex items-center gap-2">
      <div class="search-area relative w-45 sm:w-60">
        <input
          v-model="query"
          type="text"
          placeholder="搜索题目..."
        ...
        <div
          ...
          class="absolute top-10.5 left-0 right-0 rounded-lg max-h-90 overflow-y-auto z-200 shadow-xl border transition-theme"
          :style="{
