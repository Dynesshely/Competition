  ...
  <div v-if="idx" class="space-y-8">
    ...
    <section class="text-center py-8 px-4">
      ...
      <p class="flex justify-center gap-6 flex-wrap text-[#6e7681] text-sm">
        <span>📝 <strong class="text-[#c9d1d9]">{{ idx.summary.totalProblems }}</strong> 道题目</span>
        <span>✅ <strong class="text-[#c9d1d9]">{{ idx.summary.withMetadata }}</strong> 篇题解</span>
        <span>🏷️ <strong class="text-[#c9d1d9]">{{ idx.tags.length }}</strong> 个标签</span>
      </p>
    </section>
    ...
    <section>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
        ...
        >
          ...
          <span class="text-[#6e7681] text-xs whitespace-nowrap ml-2">{{ o.count }} 题</span>
        </router-link>
      </div>
    </section>
